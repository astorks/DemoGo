...
namespace DemoInfo
{
	...
	public class DemoParser : IDisposable
	{
        ...
        //public static async Task<DemoParser> LoadRemoteAsync(string url)
        //{
        //    using (var httpClient = new HttpClient())
        //    {
        //        var response = await httpClient.GetAsync(url);
        //        using (var contentStream = await response.Content.ReadAsStreamAsync())
        //            return new DemoParser(contentStream);
        //    }
        //}

        public static DemoParser LoadLocal(string path)
        {
            return new DemoParser(File.OpenRead(path));
        }
        ...
        /// <summary>
		/// Parses the header (first few hundret bytes) of the demo async. 
		/// </summary>
		public async Task<DemoHeader> ParseHeaderAsync()
        {
            return await Task.Run(() => ParseHeader());
        }
        ...
        public async Task ParseToEndAsync()
        {
            await Task.Run(() => ParseToEnd());
        }
