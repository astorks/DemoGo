	...
	public class BitArrayStream : IBitStream
	{
		...
		public void Initialize(Stream stream)
		{
			using (var memstream = new MemoryStream(checked((int)stream.Length))) {
				...
				array = new BitArray(memstream.ToArray());
			}
		...
		public string PeekBools(int length)
		{
			...
			int idx = 0;
			for (int i = Position; i < Math.Min(Position + length, array.Length); i++) {
				if (array[i])
					buffer[idx++] = 49;
				else
					buffer[idx++] = 48;
			...
			return Encoding.UTF8.GetString(buffer, 0, Math.Min(length, array.Length - Position));
		}
